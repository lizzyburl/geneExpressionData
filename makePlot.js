    ...
    //$.getJSON("http://yates.webfactional.com/studies/getChartData", data, function(d){
       $.getJSON("xyData.txt",  function(d){
		...
		for (colorCode in attributes){
			...
			.attr("height", h)
			.on("click", function(){ 
			alert("on click   " + d3.select(this).attr("id"));
			clickFunction();}, true);
		    ...
		    d.forEach(function(point,i){
				...
				//	alert(clicked);
					...
					//alert(d3.select(this).style("opacity"));
					d3.select(this).attr("r", w*h*.000012).attr("class", "offClick").style("opacity", .9).attr("stroke-width", 1.3);
		    ...
		    //d3.select(".svg" + colorCode).append("rect")
		    ...
		    // d3.select(".svg" + colorCode).append("text")
...
function brushend() {
  svg.classed("selecting", !d3.event.target.empty());
		    ...
		    */
		    ...
		    d3.select("#svg" + colorCode + gene_x + gene_y).on("mouseover", function(){
			    gene_x = geneX;
			    gene_y = geneY;
			});   
	    ...
	    function makeRadiusSmallerAgain(){
	        d3.selectAll(".offClick").attr("r", w*h*.00001).attr("class", "allCircles").style("opacity", .5).attr("stroke-width", .5);
...
function rollOn (i, point,  xScale, yScale){
    ...
    //    var box = d3.selectAll(".boxText" + colorCode);
 ...
 //d3.selectAll(".svg" + colorCode).selectAll(".boxText" + colorCode)
    ...
    // d3.selectAll(".pointText" + colorCode)
    d3.select("#svg" + colorCode + gene_x + gene_y).select(".pointText" + colorCode)
